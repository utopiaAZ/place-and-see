...
export function StabilityStatus({ goal, visible, subject = '물병' }: { readonly goal: GoalProgress; readonly visible: boolean; readonly subject?: string }) {
  if (!visible) return null;
  const percentage = Math.min(100, (goal.stableForMs / goal.requiredMs) * 100);
  const label = goal.completed ? '안정성 확인 완료!' : `${subject}가 안정적인지 확인하는 중…`;
  return (
